...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Yuankai Wang.  April 2016.
...
def test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(150.0, 200.0),  # All numbers are approximate.
                rg.Point(100.0, 250.0),
                rg.Point(50.0, 200.0),
                rg.Point(100.0, 150.0)]
    circle = rg.Circle(rg.Point(100, 200),
                       50)
...
def test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  11 chocolate3 dots.'
    window = rg.RoseWindow(400, 800, title)
    circle = rg.Circle(rg.Point(123, 654), 20)
    draw_points_on_circle(window, circle, 11, 'chocolate3')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    seq = generate_points_on_circle(circle, number_of_points)
    for k in range (len(seq)):
        a = rg.Circle(seq[k], 10)
        a.attach_to(window)
        a.fill_color = color
        seq[k].attach_to(window)
...
def test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  300 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(700, 700, title)
    circle = rg.Circle(rg.Point(300, 350), 200)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range (len(points)):
        a = rg.Line(points[k], circle.center)
...
def test_polygon():
    ...
    # Test 3:
    title = 'POLYGON tests 3'
    window = rg.RoseWindow(900, 900, title)

    circle = rg.Circle(rg.Point(400, 450), 90)
    circle.outline_thickness = 20
    polygon(window, circle, 9, 'yellow', 5)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    b = rg.Line(points[len(points) - 1], points[0])
    b.color = color
    b.thickness = thickness
    b.attach_to(window)
    for k in range (len(points) - 1):
        a = rg.Line(points[k], points[k + 1])
        a.color = color
        a.thickness = thickness
        a.attach_to(window)
    window.render()
...
def test_fancy_polygon():
    ...
    window = rg.RoseWindow(800, 800, title)

    circle = rg.Circle(rg.Point(389, 421), 150)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 30, 8, 'light blue', 4)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    for k in range (len(point)):
        if (k + hops_to_next_point) <= (len(point) - 1):
            a = rg.Line(point[k], point[k + hops_to_next_point])
        else:
            a = rg.Line(point[k], point[k + hops_to_next_point - len(point)])
        a.arrow = 'last'
        a.color = color
        a.thickness = thickness
        a.attach_to(window)
