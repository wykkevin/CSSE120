...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(contents) > volume:
            ...
            self.oricontents = contents
        self.volume = volume
        self.s = ''
        self.tempcon = ''
        self.dvolume = 0
        self.orivolume = volume
        self.history = []
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents) > self.volume:
            for i in range (len(self.contents) - self.volume):
                self.s = self.s + self.contents[i + self.volume]

            for k in range (self.volume):
                self.tempcon = self.tempcon + self.contents[k]
            self.contents = self.tempcon
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            for k in range(len(self.contents) - self.volume):
                self.s = self.s + self.contents[k + self.volume]
            for i in range(self.volume):
                self.tempcon = self.tempcon + self.contents[i]
            self.contents = self.tempcon
    ...
    def double_then_shrink(self, new_volume):
        ...
        self.double()
        if (len(self.oricontents) * 2) > self.volume:
            self.dvolume = len(self.oricontents) * 2 - self.volume
        if len(self.contents) > new_volume:
            self.dvolume = self.dvolume + (len(self.contents) - new_volume)
    ...
    def reset(self):
        ...
        TOTcontents = self.contents
        self.contents = self.oricontents
        self.volume = self.orivolume
        if TOTcontents != self.contents:
            self.history = self.history + [TOTcontents]
...
def test_steal():
    ...
    # Test 1:  Box 1 steals from 2, where Box 1 has room for all of Box 2's content
...
def test_get_history():
    ...
    # Step 3 of the test:
    box.reset()  # So now box.contents is 'Good' again and box.volume is 20 again
    h = box.get_history()
    #   h is now ['GoodGo']
    expected_contents = 'Good'
    expected_volume = 20
