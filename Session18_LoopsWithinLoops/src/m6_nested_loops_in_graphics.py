...
def draw_L(window, circle, r, c):
    ...
    xcen = circle.center.x
    ycen = circle.center.y
    rc = circle.radius
    for k in range (r + 3):
        if k < r:
            for i in range (3):
                newcircle = rg.Circle(rg.Point(xcen + i * rc * 2, ycen + k * rc * 2), rc)
                newcircle.fill_color = circle.fill_color
                newcircle.attach_to(window)
                window.render()
        else:
            for j in range (c + 3):
                newcircle = rg.Circle(rg.Point(xcen + j * rc * 2, ycen + k * rc * 2), rc)
                newcircle.fill_color = circle.fill_color
                newcircle.attach_to(window)
                window.render()
...
def draw_wall_on_right(rectangle, n, window):
    ...
    recl = rectangle.get_height()
    recw = rectangle.get_width()
    for k in range (n):
        for i in range (k + 1):
            newrec = rg.Rectangle(rg.Point(point1.x - i * recw, point1.y + k * recl),
                                  rg.Point(point2.x - i * recw, point2.y + k * recl))
