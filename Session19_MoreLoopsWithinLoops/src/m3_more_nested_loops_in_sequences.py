...
Authors: David Mutchler, Valerie Galluzzi, Mark Hays, Amanda Stouder,
         their colleagues and Yuankai Wang.  October 2015.
...
def largest_number(seq_seq):
    ...
    for k in range (len(seq_seq)):
        for i in range (len(seq_seq[k])):
            if seq_seq[k][i] > seq_seq[bigkindex][bigiindex]:
                bigkindex = k
                bigiindex = i
    return seq_seq[bigkindex][bigiindex]
...
def test_largest_negative_number():
    ...
    # Test 4:
    expected = -8
    answer = largest_negative_number(([1, 2], [-8], []))
    print('Expected and actual are:', expected, answer)

    # Test 5:
    expected = -8
    answer = largest_negative_number(([-8], [], [1, 2]))
    print('Expected and actual are:', expected, answer)
...
#         for i in range (len(seq_seq[k])):
#             if seq_seq[k][i] < 0:
#                 negaseq.append(seq_seq[k][i])
#     maxnega = 0
#     if len(negaseq) == 0:
#         return None
#     for k in range (len(negaseq)):
#         if negaseq[k] > negaseq[maxnega]:
#             maxnega = k
#     return negaseq[maxnega]
    ...
    for a in range (len(seq_seq)):
        for b in range (len(seq_seq[a])):
            bigkindex = a
            bigiindex = b
            if seq_seq[bigkindex][bigiindex] < 0:
                tempnum = seq_seq[bigkindex][bigiindex]
                break
        if tempnum < 0:
            break
    if bigkindex == None:
