...
def problem2a(circle, rectangle, window):
    ...
    line1 = rg.Line(rectangle.get_upper_right_corner(), rectangle.get_lower_left_corner())
    line1.arrow = 'last'
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    x = rect.get_upper_left_corner()
    y = rect.get_lower_right_corner()
    for k in range(n):
        rec = rg.Rectangle(rg.Point(x.x - k * delta / 2, x.y - k * delta / 2), rg.Point(y.x + k * delta / 2, y.y + k * delta / 2))
        rec.attach_to(win)
    win.render()
