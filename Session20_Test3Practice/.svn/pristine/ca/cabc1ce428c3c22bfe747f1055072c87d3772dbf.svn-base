...
def drawacirclewithline(point, radius, color, window):
...
def drawalineofcircles(point, radius, color, window, g):
    x = point.x
    y = point.y
    for k in range (g):
        drawacirclewithline(rg.Point(x + k * 2 * radius, y), radius, color, window)

def drawupward(n, dy, point, radius, color, window):
    for g in range (n):
        drawalineofcircles(point, radius, color, window, (g + 1))
        point.x = point.x - radius
        point.y = point.y - dy
    point.y = point.y + dy * n
    point.x = point.x + radius * n
...
def hourglass(window, n, point, radius, color):
    ...
    drawupward(n, radius * (3 ** 0.5), point, radius, color, window)
    drawupward(n, -radius * (3 ** 0.5), point, radius, color, window)
...
#     window.render(0.1)
#     for k in range (n - 1):
#         point.x = x - radius * (k + 1)
#         point.y = y - radius * 2 * (k + 1)
#         circle = rg.Circle(point, radius)
#         circle.fill_color = color
#         circle.attach_to(window)
#         window.render(0.1)
#         for i in range (k + 1):
#             point.x = point.x + radius * 2
#             circle = rg.Circle(point, radius)
#             circle.fill_color = color
#             circle.attach_to(window)
#             window.render(0.1)
...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range (m):
        ...
        recleftupper = rg.Point(square.center.x - (i + 1) * radius, square.center.y - radius * i * (3 ** 0.5) - radius)
        recrightdown = rg.Point(square.center.x + (i + 1) * radius, square.center.y + radius * i * (3 ** 0.5) + radius)
        rec = rg.Rectangle(recleftupper, recrightdown)
        rec.attach_to(window)
        window.render(0.1)
        n = n + 1
        square.length_of_each_side = (2 * i + 3) * radius
