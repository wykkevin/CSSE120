    ...
    for k in range (len(sequence_of_sequences)):
        for i in range (len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][i])is int:
                intlist.append(sequence_of_sequences[k][i])
    return intlist
...
def test_big_letters():
    ...
    # Test 2:
    expected = 'SDFGHDFUHYGOOPSAPSBOSCOPDOO'
    answer = big_letters([(3, 1, 4),
                          'SDFGHijuhgyhgDFuhyjghxdffUHYG',
                          (10, 'Ok what is ThiSSS?', 110),
                          [],
                          ['oops'],
                          'oops',
                          ['OOPS'],
        ...
        if type(sequence_of_sequences[k])is str:
            for i in range (len(sequence_of_sequences[k])):
                if sequence_of_sequences[k][i].isupper():
                    stringstr = stringstr + sequence_of_sequences[k][i]
    return stringstr
