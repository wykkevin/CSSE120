...
class Point(object):
    ...
    def move_to(self, xx, yy):
        self.dist = self.dist + self.get_distance_from(Point(xx, yy))
        self.x = xx
        self.y = yy
        self.countofmoves = self.countofmoves + 1
        return Point(self.x, self.y)
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.countofmoves = self.countofmoves + 1
        self.dist = self.dist + (dx ** 2 + dy ** 2) ** 0.5
        return Point(self.x, self.y)

    def get_number_of_moves_made(self):
        return self.countofmoves

    def get_distance_from(self, point):
        return ((point.x - self.x) ** 2 + (point.y - self.y) ** 2) ** 0.5

    def get_distance_from_start(self):
        return self.get_distance_from(Point(self.inx, self.iny))

    def get_distance_traveled(self):
        return self.dist

    def closer_to(self, point1, point2):
        if self.get_distance_from(point1) >= self.get_distance_from(point2):
            return point1
        else:
            return point2
