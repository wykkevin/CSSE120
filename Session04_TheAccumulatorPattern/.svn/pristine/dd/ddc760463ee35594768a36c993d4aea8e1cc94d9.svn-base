...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Yuankai Wang.  March 2016.
...
def draw_squares_from_circle(n, circle, window):
    ...
    squpoint = rg.Point(circle.center.x, circle.center.y)
    squa = circle.radius * 2
    a = rg.Square(squpoint, squa)
    a.attach_to(window)
    for _ in range(n):
        squpoint = rg.Point(squpoint.x + circle.radius, squpoint.y + circle.radius)
        b = rg.Square(squpoint, squa)
        b.attach_to(window)
...
def test_draw_circles_from_rectangle():
    ...
    # Test 1:
    rectangle = rg.Rectangle(rg.Point(400, 350), rg.Point(410, 370))
    rectangle.fill_color = 'green'
    draw_circles_from_rectangle(2, 7, rectangle, window1)

    # Test 2:
    rectangle = rg.Rectangle(rg.Point(850, 800), rg.Point(900, 780))
    rectangle.fill_color = 'red'
    draw_circles_from_rectangle(10, 20, rectangle, window1)
    window1.close_on_mouse_click()
    ...
    # Test 3:
    rectangle = rg.Rectangle(rg.Point(800, 800), rg.Point(880, 830))
    rectangle.fill_color = 'blue'
    draw_circles_from_rectangle(3, 2, rectangle, window2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    reccenter = rectangle.get_center()
    center1 = rg.Point(reccenter.x - rectangle.get_width() / 2 - rectangle.get_height() / 2, reccenter.y)
    center2 = rg.Point(reccenter.x, reccenter.y - rectangle.get_width() / 2 - rectangle.get_height() / 2)
    for _ in range(m):
        c = rg.Circle(center1, rectangle.get_height() / 2)
        center1 = rg.Point(center1.x - rectangle.get_height(), center1.y)
        c.fill_color = rectangle.fill_color
        c.attach_to(window)
    for _ in range(n):
        d = rg.Circle(center2, rectangle.get_width() / 2)
        center2 = rg.Point(center2.x, center2.y - rectangle.get_width())
        d.attach_to(window)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    e = rg.Line(rectangle1.get_center(), rectangle2.get_center())
    e.thickness = 5
    e.color = rectangle1.outline_color
    e.attach_to(window)
    for k in range(n):
        f = rg.Line(rg.Point(e.start.x - (k + 1) * 40, e.start.y + (k + 1) * 40), rg.Point(e.end.x - (k + 1) * 40, e.end.y + (k + 1) * 40))
        f.thickness = 5
        if k % 2 == 0:
            f.color = rectangle2.outline_color
        else:
            f.color = rectangle1.outline_color
        f.attach_to(window)
