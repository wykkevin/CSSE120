...
def draw_parallel_lines(n, point, length, window):
    ...
    x = point.x
    y = point.y
    pointend = rg.Point(x + length, y)
    for k in range(n):
        a = rg.Line(point, pointend)
        point = rg.Point(x , y + (30 * (k + 1)))
        pointend = rg.Point(x + length, y + (30 * (k + 1)))
        a.attach_to(window)
...
def draw_lines(n, point, window):
    ...
    x = point.x
    y = point.y
    pointend = rg.Point(x + 100, y - 100)
    b = 200 / (n - 1)
    for k in range(n):
        a = rg.Line(point, pointend)
        pointend = rg.Point(x + 100, y - 100 + (k + 1) * b)
        a.attach_to(window)
